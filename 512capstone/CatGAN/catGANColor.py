...
@tf.function #Improves performance
...
def displayImage():
    ...
    f.close()
    return Image.open("outputPhotosColor/image_at_epoch_{:04d}.png".format(epochNum))
#displayImage()
...
def main():
    ...
    np.save(os.path.join("outputColor", "trainingData"), trainingData) #save trainingData
    ...
    if os.path.exists(os.path.join("outputColor", "seed.npy")):
        ...
        np.save(os.path.join("outputColor", "seed"), seed.numpy())
    #else()
    
    tf.print(seed)

    #Create a checkpoint saving system
    checkpoint_dir = './outputColor'
    ...
    manager = tf.train.CheckpointManager(checkpoint, directory = "outputColor", max_to_keep = 3)
